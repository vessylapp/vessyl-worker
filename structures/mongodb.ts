...
class Mongo {
    ...
    async createDatabase(db: string): Promise<any> {
        if (!this.isConnected) {
            throw new Error('Not connected to MongoDB');
        }
    ...
    async createCollection(db: string, col: string): Promise<any> {
        if (!this.isConnected) {
            throw new Error('Not connected to MongoDB');
        }
        const database = await this.getDatabase(db);
        return database.createCollection(col);
    }
    ...
    async dbExists(db: string): Promise<boolean> {
        if (!this.isConnected) {
            throw new Error('Not connected to MongoDB');
        }
        const dbList = await this.client!.db().admin().listDatabases();
        return dbList.databases.some((d: any) => d.name === db);
    }

    async collectionExists(db: string, col: string): Promise<boolean> {
        if (!this.isConnected) {
            throw new Error('Not connected to MongoDB');
        }
        const collectionList = await this.client!.db(db).listCollections().toArray();
        return collectionList.some((c: any) => c.name === col);
    }
